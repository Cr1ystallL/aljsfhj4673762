...
Всё установленное — MIT, тексты лицензий лежат рядом со скиллами:

- `design-taste-frontend`, `redesign-existing-projects` — MIT, © Leonxlnx (`LICENSE` в папке скилла).
- `caveman` — MIT (`LICENSE` в папке скилла). В апстриме часть каталогов движка идёт под
  BSL-1.1, но сам скилл к ним не относится и они сюда не копировались.
- `ui-ux-pro-max` и соседние — MIT, © NextLevelBuilder, сгенерированы официальным
  `ui-ux-pro-max-cli`.
