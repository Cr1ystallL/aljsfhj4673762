...
async def show_basketball_game(callback: CallbackQuery):
    """Показать меню игры в баскетбол"""
...
@router.callback_query(F.data == "basketball_make_bet")
async def basketball_make_bet(callback: CallbackQuery):
...
@router.callback_query(F.data == "back_to_basketball")
async def back_to_basketball(callback: CallbackQuery):
...
async def cmd_basketball(message: Message):
    ...
    if not command_text:
        await message.answer(
            ...
            "Используйте: <code>/basket [сумма] [вид]</code>"
        )
        return
    ...
    try:
        ...
        if bet_type in ["попадание", "hit", "п"]:
            multiplier = 2.0
            bet_desc = "Попадание"
        elif bet_type in ["промах", "miss", "м"]:
            multiplier = 1.8
            bet_desc = "Промах"
        else:
        ...
        await message.answer(text, reply_markup=get_confirm_bet_menu(bet_id, "basketball"))
    
    except (ValueError, IndexError) as e:
        await message.answer(f"❌ <b>Ошибка:</b> {str(e)}")


@router.callback_query(F.data.startswith("confirm_basketball:"))
async def confirm_basketball_bet(callback: CallbackQuery):
    """Подтверждение ставки в баскетбол"""
    ...
    await callback.message.edit_text(
        f"Бросаем <a href='https://t.me/{bot_username}?start=basketball_{bet_id}'>мяч</a>..."
    )
    ...
    if bet_data['bet_type'] in ["попадание", "hit", "п"] and 4 <= dice_value <= 5:
        is_win = True
    elif bet_data['bet_type'] in ["промах", "miss", "м"] and 1 <= dice_value <= 3:
        is_win = True
    ...
    if is_win:
        ...
        # Сохраняем в историю
        game_history.save_game(bet_id, 'basketball', user_id, bet_data['amount'], 'win', {
        ...
        await result_msg.edit_text(
            f"Игра <a href='https://t.me/{bot_username}?start=basketball_{bet_id}'>#{bet_id}</a>"
        )
    else:
        ...
        # Сохраняем в историю
        game_history.save_game(bet_id, 'basketball', user_id, bet_data['amount'], 'loss', {
            'bet_type': bet_data['description'],
            'dice_value': dice_value,
            'multiplier': 0
        })
...
@router.callback_query(F.data.startswith("cancel_basketball:"))
async def cancel_basketball_bet(callback: CallbackQuery):
    """Отмена ставки"""
    bet_id = callback.data.split(":")[1]
    ...
    await callback.message.edit_text(
        ...
        "Вы можете сделать новую ставку командой /basket"
    )
