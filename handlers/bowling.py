...
async def show_bowling_game(callback: CallbackQuery):
    """Показать меню игры в боулинг"""
...
@router.callback_query(F.data == "bowling_make_bet")
async def bowling_make_bet(callback: CallbackQuery):
...
@router.callback_query(F.data == "back_to_bowling")
async def back_to_bowling(callback: CallbackQuery):
...
async def cmd_bowling(message: Message):
    ...
    if not command_text:
        await message.answer(
            ...
            "Используйте: <code>/bowl [сумма] [вид]</code>\n\n"
            "<b>Примеры:</b>\n"
    ...
    try:
        ...
        if bet_type in ["страйк", "strike", "6"]:
            multiplier = 5.0
            bet_desc = "Страйк (6)"
        ...
        await message.answer(text, reply_markup=get_confirm_bet_menu(bet_id, "bowling"))
    
    except (ValueError, IndexError) as e:
        await message.answer(f"❌ <b>Ошибка:</b> {str(e)}")


@router.callback_query(F.data.startswith("confirm_bowling:"))
async def confirm_bowling_bet(callback: CallbackQuery):
    """Подтверждение ставки в боулинг"""
    ...
    await callback.message.edit_text(
        f"Бросаем <a href='https://t.me/{bot_username}?start=bowling_{bet_id}'>шар</a>..."
    )
    ...
    if bet_data['bet_type'] in ["страйк", "strike", "6"] and dice_value == 6:
        is_win = True
    ...
    if is_win:
        ...
        # Сохраняем в историю
        game_history.save_game(bet_id, 'bowling', user_id, bet_data['amount'], 'win', {
        ...
        await result_msg.edit_text(
            f"Игра <a href='https://t.me/{bot_username}?start=bowling_{bet_id}'>#{bet_id}</a>"
        )
    else:
        ...
        # Сохраняем в историю
        game_history.save_game(bet_id, 'bowling', user_id, bet_data['amount'], 'loss', {
            'bet_type': bet_data['description'],
            'dice_value': dice_value,
            'multiplier': 0
        })
...
@router.callback_query(F.data.startswith("cancel_bowling:"))
async def cancel_bowling_bet(callback: CallbackQuery):
    """Отмена ставки"""
    bet_id = callback.data.split(":")[1]
    ...
    await callback.message.edit_text(
        ...
        "Вы можете сделать новую ставку командой /bowl"
    )
