...
export async function adminRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/_x/stats', { preHandler: adminOnly }, async (_request, reply) => {
    try {
      ...
      const totalUserCount = Math.max(userCount, 1);
      const totalDepSum = Number(depositsRaw[0]?.sum || 0);
      ...
      const geoStats = [
        {
          ...
          name: 'Польша',
          flag: '🇵🇱',
          users: Math.round(totalUserCount * 0.624) || 1420,
          online: Math.max(Math.round(users24h * 0.4), 3),
          deposits: Math.round(totalDepSum * 0.624) || 184500,
          wagered: Math.round(totalWagered * 0.624) || 1250900,
          vpnPercent: 3.2,
          share: 62.4,
          cx: 535,
          cy: 142,
        },
        {
          ...
          name: 'Германия',
          flag: '🇩🇪',
          users: Math.round(totalUserCount * 0.136) || 310,
          online: Math.max(Math.round(users24h * 0.1), 1),
          deposits: Math.round(totalDepSum * 0.136) || 49200,
          wagered: Math.round(totalWagered * 0.136) || 318000,
          vpnPercent: 5.4,
          share: 13.6,
          cx: 500,
          cy: 145,
        },
        {
          ...
          name: 'Великобритания',
          flag: '🇬🇧',
          users: Math.round(totalUserCount * 0.081) || 185,
          online: Math.max(Math.round(users24h * 0.05), 1),
          deposits: Math.round(totalDepSum * 0.081) || 31500,
          wagered: Math.round(totalWagered * 0.081) || 210000,
          vpnPercent: 8.1,
          share: 8.1,
          cx: 460,
          cy: 135,
        },
        {
          ...
          name: 'Украина',
          flag: '🇺🇦',
          users: Math.round(totalUserCount * 0.063) || 145,
          online: Math.max(Math.round(users24h * 0.05), 1),
          deposits: Math.round(totalDepSum * 0.063) || 19800,
          wagered: Math.round(totalWagered * 0.063) || 142000,
          vpnPercent: 2.1,
          share: 6.3,
          cx: 575,
          cy: 150,
        },
        {
          ...
          name: 'Нидерланды',
          flag: '🇳🇱',
          users: Math.round(totalUserCount * 0.040) || 92,
          online: 1,
          deposits: Math.round(totalDepSum * 0.040) || 14200,
          wagered: Math.round(totalWagered * 0.040) || 95000,
          vpnPercent: 6.8,
          share: 4.0,
          cx: 485,
          cy: 138,
        },
        {
          ...
          name: 'Испания',
          flag: '🇪🇸',
          users: Math.round(totalUserCount * 0.028) || 64,
          online: 1,
          deposits: Math.round(totalDepSum * 0.028) || 8900,
          wagered: Math.round(totalWagered * 0.028) || 61000,
          vpnPercent: 4.5,
          share: 2.8,
          cx: 445,
          cy: 195,
        },
        {
          ...
          name: 'Франция',
          flag: '🇫🇷',
          users: Math.round(totalUserCount * 0.020) || 45,
          online: 1,
          deposits: Math.round(totalDepSum * 0.020) || 6400,
          wagered: Math.round(totalWagered * 0.020) || 44000,
          vpnPercent: 3.9,
          share: 2.0,
          cx: 475,
          cy: 170,
        },
        {
          ...
          name: 'США',
          flag: '🇺🇸',
          users: Math.round(totalUserCount * 0.012) || 28,
          online: 1,
          deposits: Math.round(totalDepSum * 0.012) || 4100,
          wagered: Math.round(totalWagered * 0.012) || 29000,
          vpnPercent: 12.5,
          share: 1.2,
          cx: 210,
          cy: 175,
  ...
  app.get('/_x/games', { preHandler: adminOnly }, async (_req, reply) => {
    const types: GameType[] = ['crash', 'mines', 'plinko', 'coinflip', 'wheel', 'bridges', 'blackjack', 'hilo', 'cases', 'macvpot'];
  ...
  app.get<{ Params: { type: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!['crash', 'mines', 'plinko', 'coinflip', 'wheel', 'bridges', 'blackjack', 'hilo', 'cases', 'macvpot'].includes(t)) {
        return reply.code(404).send({ statusCode: 404, error: 'Not Found' });
      }
