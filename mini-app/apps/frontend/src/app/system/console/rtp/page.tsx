      ...
      <div className="flex flex-col gap-5">
        <div className="flex items-center justify-between">
          ...
          <HelpButton title="Что делает авто-RTP" size={12}>
            <p>
              Это <b>замкнутый контроллер прибыли</b>. Вы вводите целевое
              значение — например «казино должно заработать 5000 zł за
              сутки». Контроллер каждый раунд проверяет фактический
              финансовый результат и подкручивает дополнительный
              house edge для каждого игрока, чтобы итог шёл к цели.
            </p>
            <p>
              Режим <b>earn</b> — казино должно заработать <code>target</code>{' '}
              за период.
            </p>
            <p>
              Режим <b>give</b> — казино должно отдать игрокам{' '}
              <code>target</code> за период (хорош для бонусных кампаний).
            </p>
            <p>
              <b>Sygnał</b> в [-1; +1]: больше нуля — казино отстаёт от
              цели, контроллер увеличивает edge. Меньше — казино идёт
              впереди графика, контроллер ослабляет edge.
            </p>
            <p>
              <b>Intensity</b> 0..1 — сила воздействия. 0.5 = умеренно,
              1.0 = максимально агрессивно (бонус +/- 45% к edge).
            </p>
            <p>
              <b>Per-user bias.</b> Игрок далеко в плюсе получает чуть
              более жёсткий edge. Игрок далеко в минусе — облегчение.
              Это не «подкрутка» провабли-фейр результатов: каждый
              раунд по-прежнему генерируется честно, корректируется
              только сумма выплаты на выигрыше.
            </p>
            <p>
              <b>Сохранение пер-юзер аккумуляторов</b>: они держатся
              в Redis на протяжении окна и автоматически очищаются
              при ротации.
            </p>
          </HelpButton>
        </div>
        ...
        >
          ...
          <p className="mt-2 font-roobert text-[11px] text-whisper-gray leading-relaxed">
            Контроллер добавляет к house edge каждой выплаты значение
            из диапазона ±{(intensity * 0.45 * 100).toFixed(0)}%. Сейчас
            формула выдаёт сигнал {((status?.signal ?? 0) * intensity * 0.45 * 60).toFixed(1)}%
            добавки в среднем.
          </p>
        </Section>
