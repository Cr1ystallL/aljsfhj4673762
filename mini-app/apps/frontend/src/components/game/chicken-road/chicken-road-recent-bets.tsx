...
export interface ChickenRoadRecentBet {
  id: string;
  betAmount: number;
  multiplier: number;
  payout: number;
...
interface ChickenRoadRecentBetsProps {
  bets: ChickenRoadRecentBet[];
  currency?: string;
}

export function ChickenRoadRecentBets({ bets, currency = 'zł' }: ChickenRoadRecentBetsProps) {
  return (
            ...
            {bets.map((b) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {won ? `x${b.multiplier.toFixed(2)}` : 'Авария'}
                  </div>
                  <div
                    className={cn(
